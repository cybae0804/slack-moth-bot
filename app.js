...
function resetChannel(channel_id) {
  ...
  return "No entires in this channel.";
}
...
function help() {
  ...
  const alias = `- alias ['me' | user_id] [alias]\n  Sets the alias for a user. This alias is shared across any other channels. If no alias is passed in, it removes the alias.`;
...
function handler(body) {
  ...
  switch (command) {
    case 'add': {
      if (channel_name === 'directmessage') return 'Direct Messages cannot be added.';
      return addUser(
        response_url,
        channel_id,
